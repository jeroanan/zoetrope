...
<div ng-show="vm.ready && !vm.error">
  ...
  <h2>CPU/GPU</h2>
  zoe_global_prefs_row(Percentage of the CPUS to use, vm.prefs.max_ncpus_pct + '%',
                       Maximum CPU uage, vm.prefs.suspend_cpu_usage + `''%`'')

  zoe_global_prefs_row(Maximum number of CPUs to use, vm.prefs.max_cpus,
  ...
  <h2>Memory</h2>
  zoe_global_prefs_row(Maximum RAM to use when idle, vm.prefs.ram_max_used_idle_pct + '%',
  ...
  zoe_global_prefs_row(Run if computer in use, vm.prefs.run_if_user_active,
                       Run when idle for, vm.prefs.idle_time_to_run)

  <h2>Network</h2>
  zoe_global_prefs_row(Hangup if dialed, vm.prefs.hangup_if_dialed,
                       Confirm Before Making an Internet Connection, vm.prefs.confirm_before_connecting)

  zoe_global_prefs_row(Maximum download speed, vm.prefs.max_bytes_sec_down,
                       Maximum upload speed, vm.prefs.max_bytes_sec_up)

  zoe_global_prefs_row(Starting hour for network traffic, vm.prefs.net_start_hour,
                       Ending hour for network traffic, vm.prefs.net_end_hour)

  zoe_global_prefs_row(WiFi only?, vm.prefs.network_wifi_only)

  <h2>Battery</h2>
  zoe_global_prefs_row(Maximum battery temperature, vm.prefs.battery_max_temperature,
                       Minimum battery charge, vm.prefs.battery_charge_min_pct + '%')

  zoe_global_prefs_row(Run on batteries?, vm.prefs.run_on_batteries)

  <h2>Disk</h2>
  zoe_global_prefs_row(Maximum disk space to use, vm.prefs.disk_max_used_gb + 'GB',
                       Maximum percentage disk space to use, vm.prefs.disk_max_used_pct + '%')
                       

  zoe_global_prefs_row(Make tasks checkpoint to disk every, vm.prefs.disk_interval + ' seconds',
                       Minimum disk space to leave free, vm.prefs.disk_min_free_gb)
                       
  <h2>Misc. Settings</h2>
  zoe_global_prefs_row(daily_xfer_period_days, vm.prefs.daily_xfer_period_days,
                       Don't verify images, vm.prefs.dont_verify_images)

  zoe_global_prefs_row(mod_time, vm.prefs.mod_time,
