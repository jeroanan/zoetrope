...
** TODO Make tasks' deadline times indicate when deadline is near/exceeded :Task:
...
** TODO On mobile the title tooltip wouldn't work
So in addition we should probably make click/tap on the name toggle it to full name
and then back to truncated.
** TODO Make active task state a word rather than just a number :Task:
